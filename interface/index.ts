...
import { DeckType } from "../lib/Objects/Deck";
import { Hand } from "../lib/Objects/Player";
...
function render(ctx: CanvasRenderingContext2D, game: Game) {
  ...
  players.forEach((player, index, arr) => {
    ...
    renderHand(ctx, player.hand, index == 0, playerX, playerY);
  });
...
function renderHand(
  ...
  cards.forEach((card, index) => {
    const xOffset = (cards.length / 2 - index) * cardWidth;
    const yOffset = 0;
...
function renderCard(
  ...
  ctx.fillStyle = "black";
  ctx.font = "15px Arial";
  if (open) ctx.fillText(card.name, x + 10, y + 50);
}

export function init(): [CanvasRenderingContext2D, Game] {
  const game = new Game({
    type: "game",
    name: "test",
    description: "Eyo",
